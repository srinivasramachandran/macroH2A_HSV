...
#format: chr start_coord end_coord value
	...
	while(my $bed_line=<BEDGRAPH>){
		...
		if($bed_line=~/track/){
			...
			$temp[0]=~s/chr//;
			$chrid=$temp[0];
			my $st=( int( ($temp[1]+1) / 25 ) )*25;
			my $en=( int(  $temp[2]    / 25 ) )*25;
			for(my $i=$st;$i<=$en;$i+=25){
				$bedgraph{$chrid}{$i}=$temp[3];
			}
		}
	}
...
#format: chr start_coord end_coord value
	...
	while(my $bed_line=<BEDGRAPH>){
		...
		if($bed_line=~/track/){
			...
			my $en=$temp[2];
			for(my $i=$st;$i<=$en;$i++){
				$bedgraph{$chrid}{$i}=$temp[3] if($temp[3]!=0);
			}
		}
	}
	close(BEDGRAPH);
	...
	while(chomp(my $bed_file = <FILE>)){
		...
		while(my $bed_line=<BED>){
			...
			if($#temp < 2){
				...
				my $st=int( ( ($temp[1]+1) /$step) + 0.5 )*$step;
...
sub readbed_spike{
	my ($bed_file,$min,$max,$step,$genome,$tgs,$spike,$tot)=@_;
	my @temp; my %wig; my $bed_line; my $chrid; my %twig; my $ct=0;
	my $i; my $j;
	my %genome_size = %{$tgs};
	if($tot==0){
...
sub readbed_spike_list{
	my ($bed_file_list,$min,$max,$step,$genome,$tgs)=@_;
	my @temp; my %wig; my $bed_line; my $chrid; my %twig; my $ct=0;
	my $i; my $j;
	my %genome_size = %{$tgs};
	open(BEDLIST,$bed_file_list) || die "BED LIST $!\n";
	while(my $bed_file_line=<BEDLIST>){
		my ($bed_file,$spike) = split /[\t\n\ ]+/,$bed_file_line;
		print STDERR "Processing list: $bed_file SPIKE: $spike\n";
		if($bed_file=~/gz$/){
			open(BED,"gunzip -c $bed_file |") || die "cannot open pipe to $bed_file";
		}else{
			open(BED,$bed_file) || die "BED $!\n";
...
sub readbed_c_v2{
	...
	while(my $bed_line=<BED>){
		...
		if($#temp < 2){
			...
			my $mp=int( ( ($temp[1]+$temp[2]) /2) + 0.5 );
			if($mp<= int($genome_size{$temp[0]})){
					$twig{$chrid}{$mp}++;
					$ct++;
			}
		}
...
#wig format: Coord Value ; chr in the header only
	...
	if($_[0]=~/gz$/){
		open(WIG,"gunzip -c $_[0] |") || die "cannot open pipe to $_[0]";
	}else{
			open(WIG,$_[0]) || die "cannot open $_[0]  $!\n";
	}
	while(my $wig_line=<WIG>){
		@temp = split /[=\ \s\n\t]+/, $wig_line;
		if($wig_line=~/chrom/){
